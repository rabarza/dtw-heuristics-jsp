...
def solve_schedule(req: SolveRequest):
    ...
    schedule_df_human = add_day_hour_columns(schedule_df, H_daily_hours=req.H_daily_hours)

    # Calcular makespan
    makespan = schedule_df_human['end_time_hours'].max() if not schedule_df_human.empty else 0.0
    ...
    return SolveResponse(makespan=makespan, schedule=output)
